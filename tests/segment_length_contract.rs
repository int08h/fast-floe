...
fn release_build_enforces_documented_segment_length_bounds() {
    ...
    assert!(Parameters::with_segment_length(64).is_ok());
    assert!(Parameters::with_segment_length(u32::MAX - 1).is_ok());

    // Values just outside the documented range are rejected with the
    // offending value
    assert_eq!(
        Parameters::with_segment_length(63),
        Err(Error::InvalidSegmentLength { actual: 63 })
    );
    assert_eq!(
        Parameters::with_segment_length(u32::MAX),
